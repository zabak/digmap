...
//			System.out.println(type+" => "+clazz);
	...
	public static Document newDocument(JSONObject obj) throws DocumentCreationException {
		try {
			Document d = docTypes.get(obj.getString("_content_type")).newInstance();
			d.commonData = obj;
			d.metaData = new JSONObject();
			d.setRevision(obj.getString("_rev"));
			d.setRevisionDate(new Date( new Long(obj.getString("_rev_date")).longValue() ));
			d.setRevisionUser(obj.getString("_rev_by"));
			d.dataDirty = false;
			d.commonDirty = false;
			return d;
		} catch ( Exception e ) { }
	...
	public static Document newDocument(byte data[]) throws DocumentCreationException {
		return newDocument(JSONObject.fromString(new String(data)));
	}
	...
	public static Document newDocument(Backend backend,String database, String username) throws DocumentCreationException {
		String id=generateId(backend,database);
	...
	protected static Document newDocument(Backend backend,String database, String id, String contentType, Class<? extends Document> clazz, JSONObject commonData, String username) throws DocumentCreationException  {
		...
		try {
			...
			if (commonData==null) {
				...
				d.commonData= JSONObject.fromJSONObject(commonData);
			}
			d.metaData = new JSONObject();
