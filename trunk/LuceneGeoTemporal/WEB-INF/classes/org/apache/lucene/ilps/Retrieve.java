...
public class Retrieve {
	...
	private String analyzer_name = null;     // set through property 'analyzer'
	private String stemmer = null;  // if analyzer not set, stemmer will be used to construct a snowballAnalyzer
	private static int defaultMaxResults = Integer.MAX_VALUE;
	private final int[] docs = new int[512]; // buffered doc numbers
	private final int[] freqs = new int[512]; // buffered term freqs
	private final Vector hitListVec = new Vector();

	private LuceneRetriever retriever = null;

    private HashMap docIdMap = null;
    ...
    public String luceneId2DocId(int luceneId) {
        String docId = "";
        Document doc = null;
		try {
			doc = searcher.getReader().document(luceneId);
		} catch (IOException e) {
			e.printStackTrace();
		}
		if (doc != null) {
    ...
    public int docId2LuceneId(String docID) throws IOException {
        if (docIdMap == null) {
            //LGTE
    ...
    public int getCollectionTokenNumber() {
    	int res = -1;
    ...
    public int getDocLen(int docid) {
    	int res = -1;
    	if (docid >= 0 && searcher.getReader().numDocs() > docid) {
	    	if (searcher instanceof IndexSearcherLanguageModel) {
	    		IndexSearcherLanguageModel lms = (IndexSearcherLanguageModel)searcher;
	    		LanguageModelIndexReader lmir = lms.getLangModelReader();
	    		try {
    ...
    public int getFieldLen(String field, int docid) {
    	int res = -1;
        //LGTE
    ...
    public String hitList2String(Object[] arr) {
    ...
    public String hitListDocLength2String(Object[] arr) {
    	int estimatedBufferSize = (arr.length)*10;  
    	StringBuffer buf = new StringBuffer(estimatedBufferSize);
    	for (int i=0; i<arr.length-1;) {
    ...
    public Object[] hitListDocLength(Term term) {
    	hitListVec.clear();
   		TermDocs termDocs;
   		try {
			termDocs = searcher.getReader().termDocs(term);			
			int numDocs;
			do {
    ...
    public Object[] hitList(Term term) {
    	hitListVec.clear();
   		TermDocs termDocs;
   		try {
               //LGTE
	...
	public void setProperties() {
		...
		if (model == null) {
			throw new IllegalArgumentException("Retrieval model not set");
                ...
                // KH 2006/10/10 to support case sensitive retrieval
		analyzer_name = System.getProperty(LUCENE_PROPERTY_PREFIX + "analyzer");
	...
	public static void main(String[] args) {
		...
		if (configFileName == null) {
			usage();
                ...
                Retrieve retr = new Retrieve(configFileName);
                LuceneRetriever retriever = retr.getRetriever();
	...
	public Retrieve(String configFileName) {
        boolean flexSyntax = false;
		...
		try {
			...
			String[] stopwords =
			    ...
			    analyzer = new SnowballAnalyzer(stemmer, stopwords);
                        }
			String queryID = "";

			if (collectionType.equals("xml")) {
				retriever =
					new LuceneRetrieverXML(searcher, analyzer, defaultMaxResults);
			} else {
