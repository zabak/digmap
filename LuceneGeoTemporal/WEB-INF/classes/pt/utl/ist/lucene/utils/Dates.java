...
public class Dates
{
    ...
    public static Integer[] parseDateFields(String str)
    {
        ...
        if (str != null && str.length() > 0)
        {
            ...
            {
                if (field0Str.length() > 0)
                    month = new Integer(field0Str);
            ...
            if (day != null && day > 31)
                Dates.logger.warn("Day found bigger than 31 was:" + day);
            if (day != null && day < 1)
                Dates.logger.warn("Day found lesser than 1 was:" + day);

            if (month != null && month > 12)
                Dates.logger.warn("Month found bigger than 31 was:" + month);
            if (month != null && month < 1)
                Dates.logger.warn("Month found lesser than 1 was:" + month);
    ...
    public static GregorianCalendar getGregorianCalendar(Integer[] fields, boolean min)
    {
        ...
        if (fields == null || fields.length == 0 || fields[0] == null)
            if (min)
                gc = new GregorianCalendar(1, 0, 1);
            else
                gc = new GregorianCalendar();
        else if (fields.length == 3 && fields[2] != null)
            gc = new GregorianCalendar(fields[0], Dates.getValidMonth(fields[1]) - 1, Dates.getValidDay(fields[2]));
        else if (fields.length >= 2 && fields[1] != null)
            if (min)
                gc = new GregorianCalendar(fields[0], Dates.getValidMonth(fields[1]) - 1, 1);
            else if (fields[1] == 12)
                gc = new GregorianCalendar(fields[0], 11, 31);
            else
                gc = new GregorianCalendar(fields[0], Dates.getValidMonth(fields[1]), 1);
        else if (min)
            gc = new GregorianCalendar(fields[0], 0, 1);
        else
            gc = new GregorianCalendar(fields[0], 11, 31);
        return gc;
    }
    ...
    public static Date getMiddleDate(String startDate, String endDate)
    {
        ...
        if(gcStart == null && gcEnd == null)
            return null;
        else if(gcStart == null)
            return gcEnd.getTime();
        else if(gcEnd == null)
            return gcStart.getTime();
    ...
    public static long getMiddleDate(long startDate, long endDate)
    {
        ...
        long middle = (startDate + endDate) / 2;
    ...
    public static int getDistanceYears(String d1, String d2)
    {
        ...
        if(bottomYear == topYear)
            ...
            return Math.abs(topYear - bottomYear);
    }
