...
//        args = new String[]{"-op", "transform","-oType","other", "-xml", "D:\\Servidores\\DATA\\ntcir\\runs\\filtered_qe_t_all.xml", "-xsl","classpath://jmachado/RunId2Treceval.xsl","-o","d:\\teste.txt"};
        ...
        if(operation == Op.transform)
        {
            ...
            else
            {
                Document xmlDoc;
                if(xmlEnc != null)
                    xmlDoc = XmlUtils.parse(new FileInputStream(xmlFile), xmlEnc);
                else
                    xmlDoc = XmlUtils.parse(new FileInputStream(xmlFile));
                ...
                if(outputType == OutputType.xml && outputEnc != null && outputFile != null)
                {
                    ...
                    if(xslFile.startsWith("classpath://"))
                    {
                        ...
                        doc = XmlUtils.styleDocument(xmlDoc, xslFile,false,null);
                    XmlUtils.write(doc,new FileWriter(outputFile), outputEnc);
                }
                else if(outputType == OutputType.other && outputEnc == null && outputFile == null)
                {
                    if(xslFile.startsWith("classpath://"))
                        XmlUtils.styleDocument(xmlDoc, xslFile.substring("classpath://".length()),true,null,System.out);
                    else
                        XmlUtils.styleDocument(xmlDoc, xslFile,false,null,System.out);
                }
                else if(outputType == OutputType.other && outputFile != null)
                {
                    ...
                    if(xslFile.startsWith("classpath://"))
                        XmlUtils.styleDocument(xmlDoc, xslFile.substring("classpath://".length()),true,null,fos);
                    else
                        XmlUtils.styleDocument(xmlDoc, xslFile,false,null,fos);
                    fos.close();
                }
                else
                {
