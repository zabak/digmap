...
xmlns:bn="info:lc/xmlns/marcxchange-v1" xmlns:dc="http://purl.org/dc/elements/1.1/">
	...
	<xsl:template match="bn:record">
		...
		>
			...
			<xsl:for-each select="//bn:datafield[@tag='956']/bn:subfield[@code='a']">
				<dc:identifier xsi:type='dcterms:URI'><xsl:text>http://opac.bncf.firenze.sbn.it/mdigit/jsp/mdigit.jsp?idr=</xsl:text><xsl:value-of select="."/></dc:identifier>
			</xsl:for-each>	
			...
			<xsl:for-each select="//bn:datafield[@tag='071']/bn:subfield[@code='a']">
				<dc:identifier><xsl:text>Publishers' Numbers for Music: </xsl:text><xsl:value-of select="."/></dc:identifier>
			</xsl:for-each>						
...
<xsl:template name="unimarc2dc_qualified" xmlns:dcterms="http://purl.org/dc/terms/">
	...
	<xsl:choose>
		...
		<xsl:when test="substring( //bn:leader , 8,1)='a'"><!-- analitico periodico ou monografico-->
			<dc:description>Analitic</dc:description>
		...
		<xsl:when test="substring( //bn:leader , 8,1)='m'"><!-- monografia -->
			<xsl:choose>
			  <xsl:when test="//bn:datafield[@tag='481']">
					<dc:description>Monographic</dc:description>
			  </xsl:when>
			  <xsl:otherwise>
					<dc:description>Monographic</dc:description>
			  </xsl:otherwise>
		...
		<xsl:when test="substring( //bn:leader , 8,1)='s'"><!-- serie -->
			<dc:description>Serial</dc:description>
		</xsl:when>
	</xsl:choose>
	...
	<xsl:choose>
		<xsl:when test="substring( //bn:leader , 7,1)='a'">
			<dc:type>language materials, printed</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='b'">
			<dc:type>language materials, manuscript</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='c'">
			<dc:type>music scores, printed</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='d'">
			<dc:type>music scores, manuscript</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='e'">
			<dc:type>cartographic materials, printed</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='f'">
			<dc:type>cartographic materials, manuscript</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='g'">
			<dc:type>projected and video material</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='i'">
			<dc:type>sound recordings, nonmusical performance</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='j'">
			<dc:type>sound recordings, musical performance</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='k'">
			<dc:type>two dimensional graphics</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='l'">
			<dc:type>electronic resources</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='m'">
			<dc:type>multimedia</dc:type>
		</xsl:when> 
		<xsl:when test="substring( //bn:leader , 7,1)='r'">
			<dc:type>three dimensional artefacts and realia</dc:type>
		</xsl:when> 
	</xsl:choose>
