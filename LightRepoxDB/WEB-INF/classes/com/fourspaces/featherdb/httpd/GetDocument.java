...
public class GetDocument extends BaseRequestHandler {
	...
	public void handleInner(Credentials credentials, HttpServletRequest request, HttpServletResponse response, String db, String id, String rev/*, String[] fields*/) throws IOException{
		...
		if (d!=null) {
			...
			boolean showMeta = "true".equals(request.getParameter("meta")) | "true".equals(request.getParameter("showMeta"));
			boolean showRevisions = "true".equals(request.getParameter("revisions")) | "true".equals(request.getParameter("showRevisions"));						
			...
			if (showMeta && d.writesRevisionData()) { // only show meta data if the document doesn't write it by default
				...
				sendMetaData(response,d,request.getParameterMap(),showXML);
			} else {
				sendDocument(response,d,request.getParameterMap(),showXML);
			}
		} else {
			...
			sendError(response, "Document not found",status, HttpServletResponse.SC_NOT_FOUND,showXML);
		}
	}
