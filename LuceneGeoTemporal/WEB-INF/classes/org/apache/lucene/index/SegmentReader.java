 ...
 *
 * @version $Id: SegmentReader.java,v 1.4 2004/10/13 11:10:46 gilad Exp $
 */
final class SegmentReader extends IndexReader {
  ...
  /** Return the field length (number of terms) in a specific document.
   * If unknown field or info not stored, returns -1
   */
  public int getFieldLength(int docNumber, String field)
          throws IOException {
    // Check if this field is invalid or has no stored term vector
    FieldInfo fi = fieldInfos.fieldInfo(field);
    if (fi == null || !fi.storeTermVector) return -1;

    return termVectorsReader.getFieldLength(field,  docNumber);
  }

  /** Return the document length (total number of terms in all fields)
   * If unknown doc or info not stored, returns -1
   */
  public int getDocLength(int docNumber)
          throws IOException {
    // Check stored term vector
    if (termVectorsReader == null) return -1;

    return termVectorsReader.getDocLength(docNumber);
  }
