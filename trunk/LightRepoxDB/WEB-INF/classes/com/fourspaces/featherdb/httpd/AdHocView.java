...
public class AdHocView extends BaseRequestHandler {
	...
	public void handleInner(Credentials credentials, HttpServletRequest request, HttpServletResponse response, String db, String id, String rev/*, String[] fields*/) throws IOException{
		...
		if (src!=null && src.length()>0) {
			try {
				sendJSONString(response, AdHocViewRunner.adHocView(featherDB,db,src),showXML);
			} catch (ViewException e) {
				sendError(response,"View error",e.getMessage(),showXML);
			...
			sendError(response, "View javascript source not sent",status,HttpServletResponse.SC_BAD_REQUEST,showXML);
