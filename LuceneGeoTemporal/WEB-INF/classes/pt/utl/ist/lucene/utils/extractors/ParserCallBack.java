...
public class ParserCallBack extends HTMLEditorKit.ParserCallback {
    ...
    {
        String str = new String(text);//new String(new String(text).getBytes(),encoding);

        while(str.length() > 0 && str.charAt(0) == '>')
            str = str.substring(1);

        if(inAnchor)
            anchor.append(' ').append(text);
        if(inH1)
        {
        ...
        if(!inTitle && !garbage && !inScript){
            ...
            if(startExtracting && !stopExtracting && body)
//                if(inHeaderTag)
//                    this.textTag += " " + str; // will see in the END if wants to save text
//                else
                this.text.append(' ').append(str);
            else if(!startExtracting)
                bannedTextStart += " " + str;
            else if(stopExtracting)
                bannedTextEnd += " " + str;
        }
        if(inTitle && inHead)
            title += " " + new String(text);
        if(inScript)
            script += " " + new String(text);
    }
    public void handleComment(char[] data,int pos)
    {
        ...
        {
//            //todo D-LIB specifics implementation
//            if(dataStr.contains("Standard Copyright line") || dataStr.contains("Begin the bottom sections"))
//                stopExtracting = true;
//            else if(dataStr.contains("Begin Article"))
//            {
//                bannedTextStart = text;
//                text = "";
//            }
//            else
            coments += " " + dataStr;
...
//        String inTag = t.toString().toLowerCase();
        HtmlTag htmlTag = HtmlTag.parse(t.toString());

        if(htmlTag == HtmlTag.HEAD)
            inHead = false;
        else if(htmlTag == HtmlTag.TITLE)
            inTitle = false;
//        else if(htmlTag.isHeader())
//        {
////            inHeaderTag = false;
//
////            //todo Create a config implementation for handle Tag Text to know if the follow and tag text will be saved
////            if(!headerTag.equals("h1") && !headerTag.equals("h2"))
////                if(textTag.toLowerCase().contains("references") || textTag.toLowerCase().contains("copyright"))
////                    stopExtracting = true;
////                else
////                    text += " " + textTag;
////            else
...
////            headerTag = inTag;
////            inHeaderTag = true;
        else if(htmlTag == HtmlTag.H1)
            inH1 = false;
        else if(htmlTag == HtmlTag.H2)
            inH2 = false;
        else if(htmlTag == HtmlTag.H3)
            inH3 = false;
        else if(htmlTag == HtmlTag.H4)
            inH4 = false;
        else if(htmlTag == HtmlTag.H5)
            inH5 = false;
        else if(htmlTag == HtmlTag.H6)
            inH6 = false;
        else if(htmlTag == HtmlTag.H7)
            inH7 = false;

//        }
        else if(htmlTag == HtmlTag.SCRIPT)
            inScript = false;
        else if(htmlTag == HtmlTag.STYLE)
            garbage = false;
        else if(htmlTag == HtmlTag.A)
            inAnchor = false;
    }
        ...
        if(t.toString().toLowerCase().equals("meta"))
        {
            ...
            while (enumeration.hasMoreElements())
            {
                ...
                if(obj instanceof Attribute)
                {
                    ...
                    if(attribute.toString().compareTo("content")==0)
                        content = (String) a.getAttribute(attribute);
                    else if(attribute.toString().compareTo("name")==0){
                        name = (String) a.getAttribute(attribute);
                        if(name.compareTo("author")==0 || name.compareTo("description")==0 || name.compareTo("abstract")==0
                                || name.compareTo("keywords")==0 || name.compareTo("title")==0)
                        {
//                            toText = true;
                        }
                    }
                    ...
                    /*if(http_equiv != null && content != null &&
                   (http_equiv.toLowerCase().compareTo("content-type")==0 ||
                   http_equiv.toLowerCase().compareTo("content-encoding")==0)){

                   int indexOfCharSet = content.indexOf("charset=");
                   if (indexOfCharSet > 0) {
                   encoding = content.substring(indexOfCharSet + "charset=".length());
                   try {
                   new String("".getBytes(), encoding);
                   System.out.println("Using: " + encoding);
                   } catch (UnsupportedEncodingException e) {
                   System.out.println("ERROR: parsing html in SwingHTMLEditor" + e);
                   encoding = "iso-8859-1";
                   }
                   }
                   } */
                }
                if(http_equiv != null && http_equiv.toLowerCase().compareTo("title")==0 && content != null)
                    title += " " + content;
                else
                {
                    if(name != null && content != null){
                        ...
                        if(last != null)
                            metaTags.put(name.toLowerCase(),last + " " + content);
                        else
...
//            inHeaderTag = true;
        ...
        else
        {
            ...
            garbage = false;
            inTitle = false;
            //Don't put hear header TAG: H1 H2 ... because Headers will close in 99% of documents, they affect a lot the document in browser, so people does not forget them
        }
    }
    ...
    public Set<String> getOutLinks(String urlStr){
        ...
        for (String href : outLinks.keySet())
        {
            ...
            if (href.startsWith("http://") || href.startsWith("https://"))
                link = UrlUtil.normalizeUrlEnd(href);
            else
            {
                if (href.startsWith("/"))
                    link = UrlUtil.normalizeUrlEnd(mainSite + href);
                else
                    link = UrlUtil.normalizeUrlEnd(mainSiteAndPath + "/" + href);
            }
            if (link != null)
                links.add(link);
        }
    ...
    public String[] getMainSite(String urlStr){
        ...
        //assuming that url is good and have at least 1 char after http:// or https://
        int slashDomain = urlStr.indexOf("/",7);
        if(slashDomain>0){
            //Example http://myexample.com/example/index.html --> http://myexample.com
            mainSite = urlStr.substring(0,slashDomain);
            int questionMark = urlStr.indexOf("?");
            String path;
            if(questionMark>0)
                path = urlStr.substring(slashDomain,questionMark);
            else
                path = urlStr.substring(slashDomain);

            if(!path.endsWith("/"))
                path = path.substring(0,path.lastIndexOf("/"));

            mainSiteAndPath = mainSite + path;
            if(mainSiteAndPath.endsWith("/"))
                mainSiteAndPath = mainSiteAndPath.substring(0,mainSiteAndPath.length()-1);
        }
        else{
            mainSite = urlStr;
...
//        String mainSite = url[0];
//        String mainSiteAndPath = url[1];
//        Set wimages = new HashSet();



//        Iterator iter = images.values().iterator();
//        if (iter.hasNext()) {
//            WebImage image = (WebImage) iter.next();
//            Analyzer analyzer = LuceneUtils.getAnalyzerDontReplaceKeyWords();
//            TokenStream stream = analyzer.tokenStream(null,new StringReader(text));
//            Token token;
//            List tokenBuffer = new ArrayList();
//            try {
//                int middleToken = WebImage.SURROUND_IMAGE_WORDS/2 - 1;
//                while((token = stream.next())!=null){
//                    if(tokenBuffer.size() >= WebImage.SURROUND_IMAGE_WORDS)
//                        tokenBuffer.remove(0);
//                    tokenBuffer.add(token);
//                    if(tokenBuffer.size() >= WebImage.SURROUND_IMAGE_WORDS){
//                        if(image.getTextOffSet()>((Token)tokenBuffer.get(middleToken)).startOffset())
//                        {
//
//                        }
//                    }
...
//                while(iter.hasNext())
//                {
//                    image = (WebImage) iter.next();
//                    //hierarchy image with bufferToken
//                }
//
//            } catch (IOException e) {
//                logger.error("Getting Surrounding words with TokenStream",e);
//            }
//        }
    }
    ...
    public WebImage processImage(WebImage image,String urlStr, String imagetext){
        ...
        String link;
        if(image.getSrc().startsWith("http://") || image.getSrc().startsWith("https://"))
            link = UrlUtil.normalizeUrlEnd(image.getSrc());
        else
        {
            if(image.getSrc().startsWith("/"))
                link = UrlUtil.normalizeUrlEnd(mainSite + image.getSrc());
            else
                link = UrlUtil.normalizeUrlEnd(mainSiteAndPath + "/" + image.getSrc());
        }
