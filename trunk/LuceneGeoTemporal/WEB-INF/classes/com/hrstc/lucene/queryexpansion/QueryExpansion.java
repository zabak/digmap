...
public class QueryExpansion {
	 ...
	 * @param similarity
	 * @param analyzer
	 *            - used to parse documents to extract terms
	 * @param searcher
	 *            - used to obtain idf
	 */
	 ...
	 * @param prop
	 *            - properties that contain necessary values to perform
	 ...
	 * 
	 * @param query
	 *            - for which expansion is being performed
	 * @param hits
	 *            - to use in case <code> QueryExpansion.DOC_SOURCE_FLD </code>
	 *            is not specified
	 * @param prop
	 *            - uses <code> QueryExpansion.DOC_SOURCE_FLD </code> to
	 ...
	 * 
	 * @param docsTermsVector
	 *            of the terms of the top <code> docsRelevantCount </code>
	 ...
	 * @param beta
	 *            - factor of the equation
	 * @param docsRelevantCount
	 *            - number of the top documents to assume to be relevant
	 * @param maxExpandedQueryTerms
	 *            - maximum number of terms in expanded level1query
	 * 
	...
	{
		String termWeight = prop.getProperty("qe.field.boosted.termweight");
		...
		for(String clause: clauses)
		{
			if(clause.trim() != null)
			{
				...
				String[] termsBoost = clause.split("\\^");
				String fieldtermstr = termsBoost[0];
				String[] fieldterm = fieldtermstr.split(":");
				String field = fieldterm[0];
				String termStr = fieldterm[1];
				float boost = Float.parseFloat(termsBoost[1]);
				Term term = new Term(field, termStr);
				float idf = similarity.idf(term, searcher);
				//let's assume tf = 1 because we are not counting each term tf*idf=idf
				float weight;
				if(termWeight.equals("combineModelAndBoost"))
				{
					weight = alpha*idf*boost;
				}
				else
				{
					weight = alpha*idf;
				}
				TermQuery tq = new TermQuery(new Term(field,termStr));
				tq.setBoost(weight);
				queryTerms.add(tq);
			}
		}
...
//		else
//		{
//			List<String> fields = PropertiesUtil.getListPropertiesSuffix(prop,"field.boost.");
//			for(String field: fields)
//			{
//				float boost = PropertiesUtil.getFloatProperty(prop,"field.boost." + field);
//				int count = (int) (termCount * boost);
//				for (int i = 0; i < termCount && count > 0; i++) 
//				{
//					TermQuery termQuery = termQueries.elementAt(i);
//					if (termQuery.getTerm().field().equals(field) && termQuery.getBoost() > 0) 
//					{
//						
//						terms.add(termQuery);
//						count--;
...
//		}
		...
		for (TermQuery termQuery : terms) 
		{
			...
			// Jorge: only will be considered boost greater than 0.00001 this is
			// the threshold
			...
			if (normalizedBoost > 0) {
				...
				if ((normalizedBoost + "").indexOf("E-") < 0)
					...
					System.out.println("$$$$$$$$$BAD BOOST:" + term.text()
							+ "^" + boost + " normalized = " + normalizedBoost);
					System.exit(-1);
				}
		...
		query = LuceneVersionFactory.getLuceneVersion().parseQuery(
				qBuf.toString(), Defs.FLD_TEXT, analyzer);
